...
inclusion_criteria <- function(df.bmr.diff){
  ...
  # p divided by n 
  feature = "logp.dividedby.n"
  p.dividedby.nmax = max(exp(df.bmr.diff$logp.dividedby.n))
  thresholdvect.original = c(1e-2,3e-2,1e-1)
  thresholdvect = c(thresholdvect.original, p.dividedby.nmax+1)
  df = df.bmr.diff
  ...
  df.all$thresholdValue = as.factor(df.all$thresholdValue)
  df.all.p.dividedby.n = subset(df.all, select = c("acc.test.mean",  "auc.test.mean", "brier.test.mean", "thresholdValue","logical.threshold"))
  df.all.p.dividedby.n$feature = "p.dividedby.n"
  ...
  levels(df.all.p.dividedby.n$thresholdValue) = c("0.01","0.03","0.1","max")
  levels(df.all.Cmax$thresholdValue) = c("0.55","0.65","0.8","max")
  
  
  df.all = rbind(df.all.n, df.all.p,  df.all.p.dividedby.n, df.all.Cmax)
  ...
  mdata$feature=factor(mdata$feature, levels = c("n","p","p.dividedby.n","Cmax"))
  ...
  jpeg(filename = "Data/Pictures/Figure5_InclusionCriteria_bis.jpeg", width = 1000, height = 800)
  plot(plotmeasures)
  dev.off()
  ...
  datatest = data.frame(p.dividedby.n = exp(df.bmr.diff$logp.dividedby.n))
  p.histo.p.dividedby.n = ggplot(datatest, aes(x = p.dividedby.n)) + geom_histogram(binwidth = 0.1, alpha = 0.4) +
  ...
  df.histo.p.dividedby.n = data.frame(metafeaturevalue = exp(df.bmr.diff$logp.dividedby.n), metafeaturename = "p.dividedby.n")
  df.histo.cmax = data.frame(metafeaturevalue = df.bmr.diff$Cmax, metafeaturename = "cmax")
  df.histos = rbind(df.histo.n,
                    df.histo.p,
                    df.histo.p.dividedby.n,
                    df.histo.cmax)
  ...
  jpeg(filename = "Data/Pictures/Figure5_InclusionCriteria_bis2.jpeg", width = 900, height = 800)
  plot(plot.all)
  dev.off()
  ...
  #hist(df.bmr.diff$logp.dividedby.n)
  measure.chosen = "acc.test.mean"
  feature.chosen = "logp.dividedby.n"
  ...
  p.acc_dist = plot_grid(p.n, p.p, p.p.dividedby.n, p.Cmax,
                         p.histo.n,
                         p.histo.p,
                         p.histo.p.dividedby.n,
                         p.histo.Cmax,
