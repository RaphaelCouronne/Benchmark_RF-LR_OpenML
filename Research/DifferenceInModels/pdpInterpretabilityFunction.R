...
SubsamplePdpDistance = function(task, gridsize) {
  ...
  if (nFeatures.numeric>0) {
    ...
    for (i in c(1:nFeatures.numeric)) {
      ...
      # reweight the difference with the density
      pd.diff.numeric.l1[,i] = difference.l1 / length(difference.l1)
      pd.diff.numeric.l2[,i] = difference.l2 / length(difference.l2)
      pd.diff.numeric.linf[,i] = difference.linf / length(difference.linf)
