...
df.bmr.diff = data.frame(perfsAggr.diff,
                         ...
                         #pnum, psymbolic, pnumrate, psymbolicrate, 
                         Cmax)
...
df.plot = df.bmr.diff
df.plot$positive = sapply(logical, function(x) if(x) {("Positive")} else {"Negative"})
p <- ggplot(data = df.bmr.diff, aes(x = Cmax, y = logn, colour = target.sigma))
p <- p + geom_point(size = 1)
print(p)
...
pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "logn", fun = median)
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "logp", fun = median)
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "logdimension")
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "logpsurn", fun = median)
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "logdimensionsurn", fun = median)
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "lograpportMajorityMinorityClass")
plotPartialDependence(pd.classif)

pd.classif = generatePartialDependenceData(fit.classif.rf, task.classif, "Cmax")
plotPartialDependence(pd.classif)
