...
PartialDependancePlotExample<-function(visualize = FALSE) {
  
  ## Function definition ----
  
  PartialDependancePlot<-function(pd.plot, feature.chosen.name, title = "No title assigned"){
    ...
    #plot.PartialDependanceData = plot.PartialDependanceData + theme(legend.position="none")
  ...
  PdpAnalysis= function(n, gridsize = 20,
    ...
    # Logistic Regression depending of the chosen data set
  ...
  # Computation of pdps
  res.1 = PdpAnalysis(1e3, gridsize = 20, feature.chosen.name = "X1", datasetNumber = 1, visualize = visualize) 
  res.2 = PdpAnalysis(1e3, gridsize = 20, feature.chosen.name = "X1", datasetNumber = 2, visualize = visualize) 
