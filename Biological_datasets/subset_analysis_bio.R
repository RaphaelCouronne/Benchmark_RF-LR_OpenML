...
# for the sake of simplicity. You can do the simulation on other dataset using the parameter index.



## Load the datasets and set the parameter ----

subset_analysis_bio<- function(nCores=1, seed=1, data.id = 1471, 
                               n.simulation = 20, n.max = 5e2,
                               grid.n = c(100,1000), grid.p = c(1,3,5)) {
  ...
  #n.max = 6e2 # n.max : Max n for the tiny dataset used for the computation of p
  ...
  benchmark.sub = function(task) {
    ...
    bmr = benchmark(lrn.list, task, rdesc, measures, 
  ...
  res.subsetp = foreach (j=1:gridsize, .combine = rbind) %dopar% {
    ...
    # Get the inedxes for the n.simulation simulations
    samples = sapply(c(1:n.simulation),function(x) sample(p.dataset,p.sub))
    if (p.sub==1){samples=t(samples)}
    indexdata.experiment = data.frame(samples)
    
    for (i in c(1:n.simulation)) {
      print(paste0("Beginning iteration ","i=",i,", j=",j))
      task.data = task.tiny$env$data[,-which(names(task.tiny$env$data) %in% task.tiny$task.desc$target)]
      index.p = indexdata.experiment[,i]
      dataset.sub = data.frame(task.tiny$env$data[,index.p], target = task.tiny$env$data[[task.tiny$task.desc$target]])
  ...
  save(res.subsetp, res.subsetn, file = "Data/Simulations/Subset_Analysis_bio.RData")
}



subsetAnalysis_visualization_bio<-function() {
  load(file = "Data/Simulations/Subset_Analysis_bio.RData")
  police.size = 18
  ...
  jpeg(filename = "Data/Pictures/AdditionalFigures/Figure4_SubsetSimulation_facet_bio.jpeg", width = 1000, height = 800)
  plot(pp)
  dev.off()
