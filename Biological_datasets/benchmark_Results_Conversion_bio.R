convert_results <- function(clas_used, regis, target_path) {
  ...
  result = reduceResultsList(reg = regis)
  ...
  learner.id.tuneRanger = "classif.tuneRanger"
  ...
  save(results.nas, clas_used.nas , file = "Data/Results/results_nas_bio.RData")
  ...
  perfsAggr.TR = subset(res.perfs.df, learner.id == learner.id.tuneRanger)
  perfsAggr.RF.diff = perfsAggr.RF[,3:ncol(perfsAggr.RF)]-perfsAggr.LR[,3:ncol(perfsAggr.LR)]
  perfsAggr.RF.diff$task.id = perfsAggr.RF.diff$task.id
  perfsAggr.RF.diff$rf_type = "RF"
  perfsAggr.TR.diff = perfsAggr.TR[,3:ncol(perfsAggr.RF)]-perfsAggr.LR[,3:ncol(perfsAggr.LR)]
  perfsAggr.TR.diff$task.id = perfsAggr.TR.diff$task.id
  perfsAggr.TR.diff$rf_type = "TR"
  perfsAggr.diff = rbind(perfsAggr.RF.diff,perfsAggr.TR.diff)
  library(reshape2)
